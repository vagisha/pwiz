...
namespace pwiz.Skyline.Model.Optimization
{
    ...
    public class OptimizationLibrary : XmlNamedElement, IAuditLogComparable
    {
                ...
                foreach (PeptideGroupDocNode seq in document.MoleculeGroups)
                {
                    ...
                    foreach (PeptideDocNode peptide in seq.Children)
                    {
                        foreach (TransitionGroupDocNode group in peptide.Children)
                        {
                            ...
                            var ionMobility = group.IonMobilityAndCCS;
                            foreach (TransitionDocNode transition in group.Children)
                            {
                                ...
                                {
                                    var optimizationKey = new OptimizationKey(optType, modSeq, charge, ionMobility, transition.FragmentIonName, transition.Transition.Adduct);
        ...
        public DbOptimization GetOptimization(OptimizationType type, Target seq, Adduct charge, IonMobilityAndCCS ionMobility, string fragment, Adduct productCharge)
        {
            RequireUsable();
            var key = new OptimizationKey(type, seq, charge, ionMobility, fragment, productCharge);
            double value;
            return (_database.DictLibrary.TryGetValue(new OptimizationKey(type, seq, charge, ionMobility, fragment, productCharge), out value))
                ? new DbOptimization(key, value)
                : null;
        }

        public DbOptimization GetOptimization(OptimizationType type, Target seq, Adduct charge, IonMobilityAndCCS ionMobility)
        {
            return GetOptimization(type, seq, charge, ionMobility, null, Adduct.EMPTY);
        }
