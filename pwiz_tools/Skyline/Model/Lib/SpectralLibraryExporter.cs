    ...
    public class SpectralLibraryExporter
    {
            ...
            PeptideGroupDocNode nodePepGroup, PeptideDocNode nodePep, TransitionGroupDocNode nodeTranGroup, int replicateIndex)
        {
            ...
            if (nodePep.IsProteomic)
            {
                ...
                key = new LibKey(sequence, nodeTranGroup.PrecursorAdduct.AdductCharge, nodeTranGroup.IonMobilityAndCCS);
            }
            else
            {
                // For small molecules, the "modification" is expressed in the adduct
                key = new LibKey(nodeTranGroup.CustomMolecule.GetSmallMoleculeLibraryAttributes(), nodeTranGroup.PrecursorAdduct, nodeTranGroup.IonMobilityAndCCS);
            }
