                ...
                for (int iRow = 0; iRow < dataGridView.RowCount; iRow++)
                {
                    ...
                    foreach (bool interpolated in new[] {true, false})
                    {
                        ...
                        var chromatogramInfo =
                            chromatogramGroup.GetTransitionInfo(transition.Precursor.GetDocNode(), transition.DocNode, .0001f, interpolated ? TransformChrom.interpolated : TransformChrom.raw, null);
